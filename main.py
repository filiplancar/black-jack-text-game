...
while menu != 2:
    ...
    if menu == 1:
        ...
        class Game(Players):
            ...
            def playing(self):
                while True:
                    ...
                    for i, n in enumerate(hands_holder[str(player)]):
        ...
        print('-------')
        for player in range(1,dealing.n_of_players+1):
            game.playing()
            game.print_hands()
                ...
                for player in range(1,results.n_of_players+1):
                    if sum(hands_holder['dealer']) <= self.max_value:
                        if (sum(hands_holder[str(player)]) > sum(hands_holder['dealer']) and sum(hands_holder[str(player)]) <= self.max_value):
                            if hands_holder[str(player)] in self.ace:
                            ...
                            if hands_holder['dealer'] in self.ace and bets_holder[str(player)] not in self.ace:
                                print(f'{player}. Dealer vyhral', bets_holder[str(player)], '€')
                                    
                            elif hands_holder[str(player)] in self.ace and hands_holder['dealer'] not in self.ace:
                                print(f'{player}. hráč vyhral:', 1.5*bets_holder[str(player)], '€')
                        ...
                        if sum(hands_holder[str(player)]) > self.max_value:
                            ...
                            if hands_holder[str(player)] in self.ace and hands_holder['dealer'] not in self.ace:
                                print(f'{player}. hráč vyhral:', 1.5*bets_holder[str(player)], '€')
                            else:
                                print(f'{player}. hráč vyhral', bets_holder[str(player)], '€')

        results = Results(game.n_of_players, game.user_order, 21)
        print('VÝSLEDKY')
        print('-------')
        results.comparison()
